...
setup_content_printers <- function(pdf_mode = TRUE){
  ...
  print_skill_bars <- function(skills, out_of = 5){
    bar_color <- "#969696"
  ...
  print_text_block <- function(text_blocks, label){
    x <- filter(text_blocks, loc == label)$text
  ...
  list(
    text_block = print_text_block,
    section = print_section,
    skill_bars = print_skill_bars
  )
}